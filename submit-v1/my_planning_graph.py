...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for act in self.all_actions:
            ...
            precond_literals = [pre_cond_node.literal for pre_cond_node in pgnode.prenodes]

            for parent_node in self.s_levels[level]:

                if parent_node.literal in precond_literals:
                    pgnode.parents.add(parent_node)
                    parent_node.children.add(pgnode)

                    self.a_levels[level].add(pgnode)
    ...
    def add_literal_level(self, level):
        ...
        for act_node in self.a_levels[level - 1]:
            ...
            for effect_node in act_node.effnodes:
                act_node.children.add(effect_node)

                effect_node.parents.add(act_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_pos_literals = node_a1.action.effect_add
        a1_neg_literals = node_a1.action.effect_rem

        a2_pos_literals = node_a2.action.effect_add
        a2_neg_literals = node_a2.action.effect_rem

        # Test if the effects of node_a1 negates with the effects of node_a2
        for a1_pos in a1_pos_literals:
            if a1_pos in a2_neg_literals:
                return True

        for a1_neg in a1_neg_literals:
            if a1_neg in a2_pos_literals:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_pos_precond = node_a1.action.precond_pos
        a1_neg_precond = node_a1.action.precond_neg

        a2_pos_precond = node_a2.action.precond_pos
        a2_neg_precond = node_a2.action.precond_neg

        a1_pos_eff = node_a1.action.effect_add
        a1_neg_eff = node_a1.action.effect_rem

        a2_pos_eff = node_a2.action.effect_add
        a2_neg_eff = node_a2.action.effect_rem


        # Test if the preconditions of one node
        #  negates with the effects of the other node
        for a1_pos in a1_pos_eff:
            if a1_pos in a2_neg_precond:
                return True

        for a1_neg in a1_neg_eff:
            if a1_neg in a2_pos_precond:
                return True

        for a1_pos in a1_pos_precond:
            if a1_pos in a2_neg_eff:
                return True

        for a1_neg in a1_neg_precond:
            if a1_neg in a2_pos_eff:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_parent = node_a1.parents
        a2_parent = node_a2.parents

        for a1p in a1_parent:
            for a2p in a2_parent:
                if a1p.is_mutex(a2p):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        n1_parents = node_s1.parents
        n2_parents = node_s2.parents

        for n2p in n2_parents:

            if node_s1 in n2p.children:
                return False

        for n1p in n1_parents:

            if node_s2 in n1p.children:
                return False
